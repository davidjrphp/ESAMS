...
class Artists extends DBConnection
{
    ...
    function registration()
    {
        ...
        extract($_POST);
        $main_field = ['stage_name', 'email', 'password', 'sex', 'DOB', 'about_artist', 'type'];
        $data = "";
        ...
        if ($save) {
            ...
            $resp['aid'] = $aid;
            if (!empty($id))
                $resp['msg'] = 'Artist Details has been updated successfully';
            else
            ...
            if (!empty($aid) && $this->settings->userdata('login_type') != 1) {
                $artist = $this->conn->query("SELECT * FROM `artist_list` where id = '{$aid}' ");
